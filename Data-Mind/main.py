        ...
        if username in usernames:
            if authentication_status:
                ...
                uploaded_file = st.sidebar.file_uploader(
                    "Upload Your file", type=file_format_type
                )
                ...
                if uploaded_file is not None:
                    ...
                    if file_type == "plain":
                        seperator = st.sidebar.text_input(
                            "Please Enter what seperates your data: ", max_chars=5
                        )
                    ...
                    if "Overview" in multi_function_selector:
                        ...
                        with col1:
                            ...
                            number = round(
                                (uploaded_file.size * 0.000977) * 0.000977, 2
                            )
                            st.write(number)
                    ...
                    if "Outliers" in multi_function_selector:
                        outliers_selection = st.multiselect(
                            "Enter or select Name of the columns to see Outliers:",
                            num_category,
                        )
                    ...
                    if "Drop Categorical Rows" in multi_function_selector:
                        filter_column_selection = st.selectbox(
                            "Please Select or Enter a column Name: ",
                            options=data.columns,
                        )
                        filtered_value_selection = st.multiselect(
                            "Enter Name or Select the value which you don't want in your {} column(You can choose multiple values): ".format(
                                filter_column_selection
                            ),
                            data[filter_column_selection].unique(),
                        )

                        filtered_data = filter_data(
                            data, filter_column_selection, filtered_value_selection
                        )
                    ...
                    if "Drop Numeric Rows" in multi_function_selector:
                        ...
                        selection_range = [
                            x for x in selection_range if np.isnan(x) == False
                        ]
                        ...
                        if option == "Delete data inside the range":
                            st.write(
                                "We will be removing all the values between ",
                                int(start_value),
                                "and",
                                int(end_value),
                            )
                            num_filtered_data = num_filter_data(
                                data,
                                start_value,
                                end_value,
                                num_filter_column_selection,
                                param=option,
                            )
                        else:
                            st.write(
                                "We will be Keeping all the values between",
                                int(start_value),
                                "and",
                                int(end_value),
                            )
                        ...
                        num_filtered_export = download_data(
                            num_filtered_data, label="num_filtered"
                        )
                    ...
                    if "Rename Columns" in multi_function_selector:
                        ...
                        rename_column_selector = st.selectbox(
                            "Please Select or Enter a column Name you want to rename: ",
                            options=data.columns,
                        )
                        rename_text_data = st.text_input(
                            "Enter the New Name for the {} column".format(
                        ...
                        if st.button(
                            "Apply Changes",
                            help="Takes your data and rename the column as your wish.",
                        ):
                            rename_column = rename_columns(
                                data, st.session_state.rename_dict
                            )
                            st.write(rename_column)
                            export_rename_column = download_data(
                                rename_column, label="rename_column"
                            )
                            st.session_state.rename_dict = {}
                    ...
                    if "Handling Missing Data" in multi_function_selector:
                        handling_missing_value_option = st.radio(
                            ...
                            ("Drop Null Values", "Filling in Missing Values"),
                        )

                        if handling_missing_value_option == "Drop Null Values":
                            drop_null_values_option = st.radio(
                                "Choose your option as suted: ",
                                (
                                    "Drop all null value rows",
                                    "Only Drop Rows that contanines all null values",
                                ),
                            )
                            droped_null_value = handling_missing_values(
                                data, drop_null_values_option
                            )
                            st.write(droped_null_value)
                            export_rename_column = download_data(
                                droped_null_value, label="fillna_column"
                            )

                        elif (
                            handling_missing_value_option == "Filling in Missing Values"
                        ):
                            if "missing_dict" not in st.session_state:
                                st.session_state.missing_dict = {}

                            fillna_column_selector = st.selectbox(
                                "Please Select or Enter a column Name you want to fill the NaN Values: ",
                                options=column_with_null_values,
                            )
                            fillna_text_data = st.text_input(
                                "Enter the New Value for the {} Column NaN Value".format(
                                    fillna_column_selector
                                ),
                                max_chars=50,
                            )
                            ...
                            if st.button(
                                "Draft Changes",
                                help="when you want to fill multiple columns/single column null values so first you have to click Save Draft button this updates the data and then press Rename Columns Button.",
                            ):
                                if fillna_column_selector in num_category:
                                    try:
                                        st.session_state.missing_dict[
                                            fillna_column_selector
                                        ] = float(fillna_text_data)
                                    except:
                                        st.session_state.missing_dict[
                                            fillna_column_selector
                                        ] = int(fillna_text_data)
                                else:
                                    st.session_state.missing_dict[
                                        fillna_column_selector
                                    ] = fillna_text_data
                            ...
                            if st.button(
                                "Apply Changes",
                                help="Takes your data and Fill NaN Values for columns as your wish.",
                            ):
                                fillna_column = handling_missing_values(
                                    data,
                                    handling_missing_value_option,
                                    st.session_state.missing_dict,
                                )
                                st.write(fillna_column)
                                export_rename_column = download_data(
                                    fillna_column, label="fillna_column"
                                )
                                st.session_state.missing_dict = {}
                        ...
                        if data_wrangling_option == "Merging On Index":
                            ...
                            if data_wrangling_merging_uploaded_file is not None:
                                ...
                                merge_key_selector = st.selectbox(
                                    "Select A Comlumn by which you want to merge on two Dataset",
                                    options=same_columns,
                                )

                                merge_data = data_wrangling(
                                    data,
                                    second_data,
                                    merge_key_selector,
                                    data_wrangling_option,
                                )
                                st.write(merge_data)
                                download_data(merge_data, label="merging_on_index")

                        if data_wrangling_option == "Concatenating On Axis":
                            data_wrangling_concatenating_uploaded_file = (
                                st.file_uploader(
                                    "Upload Your Second file you want to Concatenate",
                                    type=uploaded_file.name.split(".")[1],
                                )
                            )
                            ...
                            if data_wrangling_concatenating_uploaded_file is not None:
                                second_data = seconddata(
                                    data_wrangling_concatenating_uploaded_file,
                                    file_type=data_wrangling_concatenating_uploaded_file.type.split(
                                        "/"
                                    )[
                                        1
                                    ],
                                )
                                concatenating_data = data_wrangling(
                                    data, second_data, None, data_wrangling_option
                                )
                                st.write(concatenating_data)
                                download_data(
                                    concatenating_data, label="concatenating_on_axis"
                                )

                    # ==========================================================================================================================================
                    st.sidebar.info(
                        "After using this app please Click Clear Cache button so that your all data is removed from the folder."
                    )
